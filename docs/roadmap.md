...
- **More street name combinations.** Current word banks: 12 adjectives × 10 nouns × 7 suffixes = 840 possible combinations (`STREET_ADJECTIVES`, `STREET_NOUNS`, `STREET_SUFFIXES` in personal.js), deliberately excluding "Main" and any other real/common street names to avoid accidentally generating a real address. User confirmed a modest expansion of the word banks would be welcome — no specific count requested yet, just logged as wanting more variety, with the same real-address-avoidance care as the existing lists. Taste note (2026-09-08): user specifically likes the moodier/more evocative combos this bank already produces — called out "Hidden Summit Way" (real generated output, not a fluke) as sounding "ominous" in a good way. When this expansion happens, lean into more of that atmospheric flavor (Hidden, Whispering, Foggy, Quiet-type adjectives; Hollow, Ridge, Glen-type nouns) rather than just adding more generic/cheerful ones.
...
## CSS cache-busting — DONE, 2026-09-08
Every page's three stylesheet `<link>` tags now carry `?v=1` (e.g. `css/components.css?v=1`), so a browser that's cached an old CSS file picks up the new one instead of silently keeping stale styles after an edit -- this bit us on the header-centering fix (looked like nothing changed, even in incognito/other browsers, because the CSS file itself was cached by URL). **Convention going forward: bump the `?v=N` on all five pages whenever a CSS file changes**, so this doesn't recur. Also fixed same day: the landing page header (title/subtitle/theme toggle) is now centered and capped to the same 900px width as the card grid below it, instead of sitting left-aligned above a centered grid.
